...
class TestViewModel(
    ...
    fun start(subject: String, chapterId: String? = null, mode: String = "TEST50",
               questionCount: Int = 50, timeMinutes: Int = 60) {
        viewModelScope.launch {
            _state.value = TestUiState(loading = true)
            val profile = profileRepo.profileFlow().first()
            this@TestViewModel.subject = subject
            this@TestViewModel.classLevel = profile?.classLevel ?: 12
            this@TestViewModel.board = profile?.board ?: "CBSE"
            this@TestViewModel.mode = mode
            this@TestViewModel.chapterId = chapterId?.takeIf { it != "none" }

            val pool = if (this@TestViewModel.chapterId != null) contentRepo.mcqsForChapter(this@TestViewModel.chapterId!!)
                       else contentRepo.mcqsForSubject(subject, this@TestViewModel.classLevel, this@TestViewModel.board)
