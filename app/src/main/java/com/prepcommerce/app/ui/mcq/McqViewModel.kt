...
class McqViewModel(
    ...
    fun load(mode: String, subject: String, chapterId: String? = null) {
        viewModelScope.launch {
            ...
            val cleanChapterId = chapterId?.takeIf { it != "none" }
            val qs = when (mode) {
                "CHAPTER" -> cleanChapterId?.let { contentRepo.mcqsForChapter(it) } ?: emptyList()
    ...
    fun next() {
        ...
        if (s.index < s.questions.size - 1) {
            _state.value = s.copy(index = s.index + 1, selected = null, submitted = false, reported = false)
            checkBookmark()
        }
    }
    fun previous() {
